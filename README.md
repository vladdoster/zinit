...
| `nocompile`    | Don't try to compile `pick`-pointed files. The bang form (`nocompile'!'`) does **not** force compilation – it **defers** it to run after `make''` and `atclone''`/`atpull''`, so files **created by a `make`/`atclone`/`atpull` hook** (e.g. a Makefile install step, or a script generated in `atclone''`) exist before they are compiled. Point `pick''` at the location of the installed/generated file. Bare `nocompile` disables compilation entirely. See *Order of Execution*.                                                                                                                                                                                                                                                                                                                                                                                                         |
...
`compile (default)` -> `make!` -> `atclone`/`atpull` -> `make` -> `compile (deferred, nocompile'!')` ->
...
ice). Bare `nocompile` disables compilation entirely.
